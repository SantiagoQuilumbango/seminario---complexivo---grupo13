...
def imputar_anio_construccion(df):
    ...
    df_imputado["construction_year"] = df_imputado.groupby("neighbourhood")["construction_year"].transform(
    ...
    df_limpio = df_imputado.dropna(subset=["construction_year"], how="all").reset_index(drop=True)
...
def imputacion_reviews_y_precios(df):
    ...
    df_imputado["reviews_per_month"] = df_imputado.groupby("neighbourhood_group")["reviews_per_month"].transform(rellenar_mediana_columna)
    ...
    filas_antes = len(df_imputado)
    df_limpio = df_imputado.dropna(subset=["price", "reviews_per_month"], how="any").reset_index(drop=True)
    filas_despues = len(df_limpio)
